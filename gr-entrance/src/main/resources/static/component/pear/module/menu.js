layui.define(['table', 'jquery', 'element'], function (exports) {
    ...
    pearMenu.prototype.render = function (opt) {
        ...
        if (option.async) { //请求菜单列表
            if (option.method === "GET") {
                getData(option.url).then(function (data) {
                    option.data = data.menuList;
                    renderMenu(option);
                });
            } else {
    ...
    function createMenu(option) {
        ...
        $.each(option.data, function (i, item) {
            ...
            if (item.type == 0) {
                // 创 建 目 录 结 构
                content += '<a  href="javascript:;" menu-type="' + item.type + '" menu-id="' + item.menuId + '" href="' + href +
                    '" ' + target + '><i class="' + item.icon + '"></i><span>' + item.title +
                    '</span></a>';
            } else if (item.type == 1) {
                content += '<a class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.url + '" menu-id="' +
                    item.menuId +
    ...
    function createMenuAndControl(option) {
        ...
        $.each(option.data, function (i, item) {
            ...
            if (i === option.defaultMenu) {
                controlItem = '<li pear-href="' + item.url + '" pear-title="' + item.title + '" pear-id="' + item.menuId +
                    '" class="layui-this layui-nav-item"><a href="#">' + item.title + '</a></li>';
                menuItem = '<ul  pear-id="' + item.menuId + '" lay-filter="' + option.elem +
                ...
                controlItemPe += '<dd  pear-href="' + item.url + '" pear-title="' + item.title + '" pear-id="' + item.menuId +
                ...
                menuItem = '<ul style="display:none" pear-id="' + item.menuId + '" lay-filter="' + option.elem +
                    '" class="layui-nav arrow layui-nav-tree pear-nav-tree">';
            ...
            $.each(item.children, function (i, note) {
                ...
                if (note.type == 0) {
                    // 创 建 目 录 结 构
                    content += '<a  href="' + href + '" ' + target + ' menu-type="' + note.type + '" menu-id="' + note.menuId +
                        '" ><i class="' + note.icon + '"></i><span>' + note.title +
                        '</span></a>';
                } else if (note.type == 1) {
                    // 创 建 菜 单 结 构
                    content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.url +
                        '" menu-id="' + note.menuId +
    ...
    function loadchild(obj) {
        ...
        if (obj.children != null && obj.children.length > 0) {
            ...
            $.each(obj.children, function (i, note) {
                ...
                if (note.type == 0) {
                    // 创 建 目 录 结 构
                    content += '<a ' + target + '  href="' + href + '" menu-type="' + note.type + '" menu-id="' + note.menuId +
                        '"><i class="' + note.icon + '"></i><span>' + note.title + '</span></a>';
                } else if (note.type == 1) {
                    // 创 建 菜 单 结 构
                    content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.url +
                        '" menu-id="' + note.menuId + '" menu-title="' + note.title + '" menu-icon="' + note.icon + '" href="' + href +
                        '" ><i class="' + note.icon + '"></i><span>' + note.title + '</span></a>';
                }
                // 加 载 子 项 目 录
                content += loadchild(note);
